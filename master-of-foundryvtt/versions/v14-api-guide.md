...
> **Official URL:** [https://foundryvtt.com/api/v14/](https://foundryvtt.com/api/v14/)  
> **Source:** Foundry Gaming LLC Client-Side API Documentation - Version 14

---

## 📌 Overview & Scope
Welcome to the comprehensive technical documentation for the client-side API of **Foundry Virtual Tabletop Version 14**.
...
Data in Foundry Virtual Tabletop is organized around the concept of Documents. Each document represents a single coherent piece of data which represents a specific object within the framework. The term Document refers to both the definition of a specific type of data (the Document class) as well as a uniquely identified instance of that data type (an instance of that class). Foundry has over 30 distinct document types, each of which are available in the global namespace as well as under foundry.documents. Packages can extend and replace these by modifying the global  CONFIG object. Each document type can only have one registered class.

For more details on how Documents and Data Models work, see the documents and data namespaces.

## Document Abstraction

- foundry.abstract.DataModel - The abstract base class which defines a data model with corresponding schema and state.
- foundry.abstract.Document - The abstract base class shared by both client and server-side which defines database interactions and the document hierarchy.
- foundry.ClientDocument - The abstract mixed class that defines the properties added to all client documents.
- foundry.CanvasDocument - The abstract mixed class that defines the properties for documents visible on a scene.
- foundry.abstract.TypeDataModel - The abstract base class for packages defining document subtypes.
...
Foundry Virtual Tabletop includes the following primary Document types, each of which is saved to its own database table with within the active World.  CONST.PRIMARY_DOCUMENT_TYPES exists as a referenceable list of these document types. See also  CONST.COMPENDIUM_DOCUMENT_TYPES for the list of document types that can have their own compendium.
...
In addition to the above primary document types, each of which are indexed within their own tables, there are several additional document types which only exist as embedded documents within a parent Document. These embedded documents cannot exist on their own (outside of the context of a parent).  CONST.EMBEDDED_DOCUMENT_TYPES exists as a referenceable list of these document types. Embedded documents are always stored within an EmbeddedCollection instance, e.g. actor.effects will return an EmbeddedCollection that contains ActiveEffects. This embedded collection provides methods for accessing and iterating through the embedded documents.
...
The following classes provide high-level building blocks for defining HTML applications within Foundry Virtual Tabletop. More details on using applications can be found on the  foundry.applications page.
...
The primary point of interaction for community packages with the foundry codebase is via hooks. You can see the full list of hooks in the hookEvents namespace.
...
- **API Reference:** [https://foundryvtt.com/api/v14/](https://foundryvtt.com/api/v14/)
